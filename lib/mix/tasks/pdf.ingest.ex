defmodule Mix.Tasks.Pdf.Ingest do
  @moduledoc """
  ...
  @shortdoc "Run full PDF ingestion pipeline (OCR → Normalize → Export)"
  ...
  def run(args) do
    ...
    pdf_path =
      case positional do
        [path | _] -> path
        [] -> Mix.raise("Usage: mix pdf.ingest <pdf_path> [options]")
      end
    ...
    Mix.shell().info("Starting PDF ingestion pipeline...")
    ...
    case PdfPipeline.Pipeline.run(pdf_path, metadata, pipeline_opts) do
      {:ok, result} ->
        ...
        PdfPipeline.Stages.WorkDir.list_artifacts(slug)
