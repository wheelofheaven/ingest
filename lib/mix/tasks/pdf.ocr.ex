defmodule Mix.Tasks.Pdf.Ocr do
  @moduledoc """
  ...
  ## Usage
      ...
      mix pdf.ocr path/to/book.pdf --slug my-book

  If a sidecar JSON exists alongside the PDF, the slug is loaded automatically:

      mix pdf.ocr path/to/book.pdf
  """

  use Mix.Task

  @shortdoc "Extract text from a PDF via GLM-OCR"
  ...
  def run(args) do
    ...
    pdf_path =
      case positional do
        [path | _] -> path
        [] -> Mix.raise("Usage: mix pdf.ocr <pdf_path> [--slug <slug>]")
      end
    ...
    if PdfPipeline.Stages.OCR.done?(slug) && !opts[:force] do
      Mix.shell().info("OCR output already exists for '#{slug}'. Use --force to re-extract.")
      Mix.shell().info("  Path: #{PdfPipeline.Stages.WorkDir.artifact(slug, "ocr.md")}")
