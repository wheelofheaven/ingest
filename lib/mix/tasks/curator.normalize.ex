defmodule Mix.Tasks.Curator.Normalize do
  @moduledoc """
  ...
  ## Usage
      ...
      mix curator.normalize --slug my-book --code MYBOOK --lang fr --rules raelian

  If a sidecar JSON exists for the original PDF, metadata is loaded automatically:

      mix curator.normalize --slug my-book
  ...
  def run(args) do
    ...
    unless Curator.Stages.OCR.done?(slug) do
      Mix.raise("No OCR output found for '#{slug}'. Run `mix curator.ocr` first.")
    end

    if Curator.Stages.Normalize.done?(slug) && !opts[:force] do
      Mix.shell().info("Normalized output already exists for '#{slug}'. Use --force to re-run.")
      Mix.shell().info("  Path: #{Curator.Stages.WorkDir.artifact(slug, "book.json")}")
      ...
      case Curator.Stages.Normalize.run(slug, metadata, normalize_opts) do
        {:ok, result} ->
          book = result.book
          ...
          Mix.shell().info("")
          Mix.shell().info("Next step: mix curator.translate --slug #{slug}")
  ...
  defp return_existing(slug) do
    case Curator.Stages.Normalize.load(slug) do
      {:ok, json} ->
