defmodule Curator.Refiner.Strategy do
  @moduledoc """
  ...
  alias Curator.Schema.{Book, Paragraph}
  alias Curator.Refiner.LlmClient
  ...
  def refine(%Book{} = book, opts \\ []) do
    threshold = opts[:threshold] ||
      Application.get_env(:curator, :llm_confidence_threshold, 0.7)
  ...
  def refinement_stats(%Book{} = book, threshold \\ 0.7) do
    all_paragraphs = Enum.flat_map(book.chapters, &Curator.Schema.Chapter.all_paragraphs/1)
  ...
  defp extract_known_speakers(%Book{} = book) do
    book.chapters
    |> Enum.flat_map(&Curator.Schema.Chapter.all_paragraphs/1)
    |> Enum.map(& &1.speaker)
