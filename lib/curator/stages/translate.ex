defmodule Curator.Stages.Translate do
  @moduledoc """
  Stage 3: Translation.
  ...
  alias Curator.Stages.{Normalize, WorkDir}
  alias Curator.Translator
  ...
  def run(slug, opts \\ []) do
    langs = Keyword.get(opts, :langs,
      Application.get_env(:curator, :default_languages, ~w(en fr de es ru ja zh)))
