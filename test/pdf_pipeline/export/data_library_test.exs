defmodule PdfPipeline.Export.DataLibraryTest do
  use ExUnit.Case, async: true

  alias PdfPipeline.Schema.{Book, Chapter, Paragraph}
  alias PdfPipeline.Export.{SingleFile, SplitFile, DataLibrary}
  ...
  describe "SingleFile.write/2" do
    test "writes a single JSON file" do
      dir = Path.join(System.tmp_dir!(), "pdf_pipeline_test_#{System.unique_integer([:positive])}")
  ...
  describe "Validator" do
    test "validates correct book JSON" do
      json = Book.to_json(@test_book)
      assert {:ok, _} = PdfPipeline.Schema.Validator.validate_book(json)
    end

    test "rejects invalid book JSON" do
      json = %{"invalid" => true}
      assert {:error, _errors} = PdfPipeline.Schema.Validator.validate_book(json)
    end
  end
